...
let servers = process.argv.slice(3).filter(x => !x.match(/^[0-9]+$/)); // {time: number, itemset: string[]}[]

const ONE_HOUR = 60 * 60 * 1000; // ms
const PERIOD = +process.argv[process.argv.length - 1] || 24; // Un jour par défaut
...
let evalcrits = (rule, day, beginning) => {
    ...
    for (let tr of day) {
        ...
        while (k < t.length && !is_superset(cl, r)) {
            cl = new Set([...cl, ...t[k]]); // On a besoin de savoir si les éléments de b occurent dans la séquence entière
            cl2 = new Set([...cl2, ...t[k]]);
            ++k;
        }
...
let period = [beg, last];
let markers = [];
for (let i = 0; period[0] + i * ONE_HOUR * PERIOD < period[1]; ++i)
    markers.push({
        beg: period[0] + i * ONE_HOUR * PERIOD,
        end: period[0] + (i + 1) * ONE_HOUR * PERIOD
    });
...
let k = rules.map(r => ({
    ...
    servers: serverdata.map((timeline, i) => {
        let sects = markers
            ...
            .filter(p => p.s < p.e); // Garder les périodes valides
        return {
            name: path.basename(servers[i]),
